        ...
        Try
            ...
            Else
                sql = $"insert into tb_clientes (cpf,nome,data_nasc,fone,email,foto) values
                        ('{txt_cpf.Text}', '{txt_nome.Text}', '{cmb_data_nasc.Value.ToShortDateString}', '{txt_fone.Text}',
                ...
                txt_fone.Clear()
                txt_email.Clear()
                img_foto.Load(Application.StartupPath & "\Fotos\Icon_User.png")
                txt_cpf.Focus()
            End If
        Catch ex As Exception
        ...
        Try
            With dgv_dados
                ...
                ElseIf .CurrentRow.Cells(4).Selected = True Then
                    aux_cpf = .CurrentRow.Cells(1).Value
                    resp = MsgBox("Deseja realmente excluir o cpf?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "ATENÇÃO")
